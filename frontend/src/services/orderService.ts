...
export const bulkUpdateOrders = async (
  orderIds: number[],
  updateData: Partial<{ order_status_id: number | null; payment_status_id: number | null; payment_method_id: number | null; delivery_method_id: number | null; delivery_status_id: number | null; tracking_number: string | null; notes: string | null; priority: number; }>
): Promise<{ updated_count: number }> => {
  try {
    const response = await axios.post('/api/orders/bulk-update', updateData, { params: { order_ids: orderIds } });
    return response.data;
  } catch (error) {
