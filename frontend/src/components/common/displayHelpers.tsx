...
// ── Які фото показувати у ШТАТНОМУ перегляді ────────────────────────────────
...
/**
 * Кадри для ВЕРХНЬОЇ галереї картки — тієї, що показує товар як його побачить
 * покупець.
 *
 * ⚠️ У картці ДВІ галереї, і приховане їм потрібне по-різному. Ця — штатний
 * перегляд, і сховане тут не місце. Нижня (менеджер фото) навпаки ПОКАЗУЄ
 * сховане сірим, інакше його неможливо було б повернути. Саме тому правило
 * живе окремою функцією: спільний список `allImages` годує обидві, і різницю
 * легко загубити.
 */
export function visibleGalleryPhotos<T extends GalleryPhoto>(
    all: T[], activeKind: string, showDefects: boolean,
): T[] {
    return all.filter((i) => {
        if (i.hidden) return false;
        const k = i.kind ?? 'official';
...
// ── Гортання карток, коли відкритий товар вибув зі списку ───────────────────

/**
 * Індекс, ВІД ЯКОГО гортати, коли відкритого товару в списку вже немає.
 *
 * Це трапляється щоразу, коли дія в картці виводить товар з-під фільтра:
 * прийняв пропозиції під «з пропозиціями», продав під «непродані». Список
 * перечитується без нього, і навігація без памʼяті про позицію мовчки
 * зупинялась — доводилось закривати картку й відкривати наступну руками.
 *
 * Повертає синтетичний `idx`, до якого викликач додає `dir`:
 *   • товар стояв усередині сторінки → на його місце зсунувся наступний;
 *     «далі» веде на нього, «назад» — на того, що стояв перед ним;
 *   • товар був останнім на сторінці → «далі» виходить за межу (наступна
 *     сторінка), «назад» веде на нового останнього.
 */
export function vanishedItemAnchor(pageLength: number, lastKnownIdx: number, dir: 1 | -1): number {
    if (lastKnownIdx < pageLength) {
        return dir === 1 ? lastKnownIdx - 1 : lastKnownIdx;
    }
    return dir === 1 ? pageLength - 1 : pageLength;
}
