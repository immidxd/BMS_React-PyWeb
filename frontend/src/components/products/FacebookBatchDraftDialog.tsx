...
interface Props {
  ...
  busy?: boolean;
  onCancel: () => void;
  onPublish?: (request: FacebookBatchRequest) => void;
}

export interface FacebookBatchRequest {
  batch_id: string;
  items: { product_id: number; payload: FacebookDraftPayload }[];
}
...
const FacebookBatchDraftDialog: React.FC<Props> = ({ productIds, busy = false, onCancel, onPublish }) => {
  ...
  useEffect(() => {
    ...
    setLoadError(null);
    setSummary(null);
    fetch('/api/publications/facebook/preview-posts-batch', {
      method: 'POST',
      ...
      .then(async response => {
        ...
        const next = (result.items || []).map((item: any): DraftCard => ({
          ...
          draft: facebookDraftFromPreview(item.preview),
  ...
  const togglePhoto = (productId: number, imageIndex: number) => {
    updateCard(productId, card => {
      ...
      const defaultZoom = facebookDefaultZoom(card.preview, card.draft.publish_type, imageIndex, card.draft.feed_preset);
  ...
  const validateBatch = async () => {
    ...
    setSummary(null);
    try {
      const response = await fetch('/api/publications/facebook/dry-run-batch', {
        ...
        <div className="overflow-y-auto p-4 sm:p-5">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {cards.map(card => {
                const limit = facebookCaptionLimit(card.preview, card.draft.publish_type);
                const tooLong = card.draft.publish_type === 'story'
                  ? card.draft.story_text.length > (card.preview.story_text_limit || 320)
                  : card.draft.caption.length > limit;
                const preview = card.preview;
                return (
                  <article key={preview.product_id} className={`rounded-xl border p-3 transition ${card.enabled ? 'border-blue-200 bg-blue-50/20 dark:border-blue-900/70 dark:bg-blue-950/10' : 'border-gray-200 opacity-70 dark:border-gray-700'}`}>
                    <div className="flex items-start gap-3">
                      <input type="checkbox" checked={card.enabled} onChange={event => updateCard(preview.product_id, current => ({ ...current, enabled: event.target.checked }))}
                        disabled={!preview.image_count} className="mt-2 rounded border-gray-300 text-[#1877F2] focus:ring-blue-500" />
                        ...
                        <div className="mt-3 grid gap-3 md:grid-cols-[220px_1fr]">
                            ...
                            {preview.image_urls.slice(0, 10).map((url, index) => {
                              ...
                              return (
                                <button key={`${url}-${index}`} type="button" disabled={!card.enabled} onClick={() => togglePhoto(preview.product_id, index)}
                                  className={`relative aspect-square overflow-hidden rounded-md border-2 disabled:opacity-50 ${order >= 0 ? 'border-[#1877F2]' : 'border-transparent'}`}>
                                  <SmartImage src={url} alt={`Фото ${index + 1}`} thumb={96} thumbOnly className="h-full w-full" />
                                  {order >= 0 && <span className="absolute left-0.5 top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#1877F2] px-0.5 text-[9px] font-bold text-white">{order + 1}</span>}
                                </button>
                              );
                            })}
                          </div>
                          <div>
                            <textarea value={card.draft.publish_type === 'story' ? card.draft.story_text : card.draft.caption} disabled={!card.enabled} rows={5} onChange={event => updateCard(preview.product_id, current => ({ ...current, draft: current.draft.publish_type === 'story' ? { ...current.draft, story_text: event.target.value } : { ...current.draft, caption: event.target.value } }))}
                              className={`w-full resize-y rounded-lg border bg-white px-2.5 py-2 text-xs leading-relaxed text-gray-800 outline-none disabled:opacity-50 dark:bg-gray-800 dark:text-gray-100 ${tooLong ? 'border-red-400' : 'border-gray-200 focus:border-[#1877F2] dark:border-gray-700'}`} />
                            <div className={`mt-1 text-right text-[10px] ${tooLong ? 'font-semibold text-red-500' : 'text-gray-400'}`}>{card.draft.publish_type === 'story' ? `${card.draft.story_text.length}/${preview.story_text_limit || 320} · текст буде на кадрі` : `${card.draft.caption.length}/${limit}`}</div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 bg-gray-50/70 px-5 py-3.5 dark:border-gray-800 dark:bg-gray-950/30">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={validateBatch} disabled={loading || validating || busy || !enabled.length || invalidEnabled}
              className="inline-flex items-center gap-2 rounded-lg bg-[#0B5FCC] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
              {validating ? <LoadingOutlined /> : <SafetyCertificateOutlined />}
              Перевірити пакет без надсилання
            </button>
            {liveReady && onPublish && <button type="button" onClick={() => onPublish({ batch_id: typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `facebook-batch-${Date.now()}`, items: enabled.map(card => ({ product_id: card.preview.product_id, payload: card.draft })) })}
              disabled={loading || validating || busy || !enabled.length || invalidEnabled}
              className="inline-flex items-center gap-2 rounded-lg bg-[#1877F2] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
...
export default FacebookBatchDraftDialog;
