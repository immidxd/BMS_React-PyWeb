...
interface Props {
  ...
  busy?: boolean;
  onCancel: () => void;
  onConfirm?: (payload: FacebookDraftPayload) => void;
}

export const FacebookMark: React.FC<{ className?: string }> = ({ className = '' }) => (
  <span
    aria-hidden="true"
    className={`inline-flex items-center justify-center rounded-md bg-[#1877F2] font-black text-white ${className}`}
  >
    f
  </span>
);

/** Ліміт тексту залежить від типу: у стрічці Сторінки він величезний, а опис
 *  Reel Meta ріже на 2200. Показувати «0/63206» над полем Reel було б брехнею. */
export function facebookCaptionLimit(
  preview: FacebookPreview,
  publishType: FacebookDraftPayload['publish_type'],
): number {
  if (publishType === 'reel') return preview.reel_description_limit || 2200;
  return preview.caption_limit;
}

export function facebookDefaultZoom(
  preview: FacebookPreview,
  publishType: FacebookDraftPayload['publish_type'],
  imageIndex: number,
...
export function facebookDraftFromPreview(preview: FacebookPreview): FacebookDraftPayload {
  return {
    ...
    frames: preview.default_image_idx.slice(0, preview.carousel_limit).map(image_idx => ({
      image_idx,
      zoom: facebookDefaultZoom(preview, 'feed', image_idx),
      x: 0,
      y: 0,
    })),
...
const FacebookPublishDialog: React.FC<Props> = ({ data, busy = false, onCancel, onConfirm }) => {
  const [draft, setDraft] = useState<FacebookDraftPayload>(() => facebookDraftFromPreview(data));
  ...
  const preset = draft.publish_type === 'feed'
    ...
    : data.story_preset;
  const captionLimit = facebookCaptionLimit(data, draft.publish_type);
  const captionTooLong = draft.caption.length > captionLimit;
  ...
  const toggleImage = (index: number) => {
    ...
    setDraft(current => {
      ...
      const defaultZoom = facebookDefaultZoom(data, current.publish_type, index, current.feed_preset);
      return { ...current, image_idx: [...current.image_idx, index], frames: [...current.frames, { image_idx: index, zoom: defaultZoom, x: 0, y: 0 }] };
    });
  };

  const setPublishType = (publish_type: FacebookDraftPayload['publish_type']) => {
    setValidation(null);
    setDraft(current => {
      ...
      return {
        ...
        frames: image_idx.map(image_idx => ({
          ...
          zoom: facebookDefaultZoom(data, publish_type, image_idx, current.feed_preset),
        })),
      };
    });
  };
  ...
  const setFeedPreset = (feed_preset: string) => {
    ...
    setDraft(current => ({
      ...
      frames: current.frames.map(frame => {
        const previousDefault = facebookDefaultZoom(data, 'feed', frame.image_idx, current.feed_preset);
        if (Math.abs(frame.zoom - previousDefault) > 0.0001) return frame;
        return { ...frame, zoom: facebookDefaultZoom(data, 'feed', frame.image_idx, feed_preset) };
      }),
    }));
  };
  ...
  const validate = async () => {
    ...
    setValidation(null);
    try {
      const response = await fetch('/api/publications/facebook/dry-run', {
        method: 'POST',
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setPreviewBusy(true);
      try {
        const response = await fetch('/api/publications/facebook/render-preview', {
      ...
      <div className="relative flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="truncate text-base font-semibold text-gray-900 dark:text-gray-50">Facebook-чернетка · #{data.productnumber}</h2>
        ...
        <div className="overflow-y-auto p-4 sm:p-5">
          <div className={`mb-4 flex gap-2 rounded-xl border px-3 py-2.5 text-xs leading-relaxed ${liveReady ? 'border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-900/20 dark:text-green-200' : 'border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200'}`}>
            ...
            <span>{liveReady
              ? <><b>Захищене підключення готове.</b> Публікація піде через Cloudflare Worker; Page access token не потрапляє у BMS.</>
              : <><b>Безпечний режим:</b> редактор і повна перевірка готові, але надсилання заблоковане до завершення OAuth/Cloudflare.</>}
            </span>
          </div>

          <div className="mb-4 grid grid-cols-3 gap-2">
            {Object.entries(data.publish_types || { feed: { label: 'Пост / альбом', max_media: 10 }, story: { label: 'Story', max_media: 1 }, reel: { label: 'Reel зі слайдів', max_media: 10 } }).map(([key, value]) => (
              <button key={key} type="button" onClick={() => setPublishType(key as FacebookDraftPayload['publish_type'])}
                className={`rounded-xl border px-3 py-2.5 text-sm transition ${draft.publish_type === key ? 'border-[#1877F2] bg-blue-50 font-semibold text-[#1877F2] dark:bg-blue-900/20 dark:text-blue-300' : 'border-gray-200 text-gray-600 hover:border-blue-300 dark:border-gray-700 dark:text-gray-300'}`}>
                {value.label}
              </button>
            ))}
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(300px,0.9fr)_minmax(420px,1.1fr)]">
            <section>
              ...
              <div className="mt-2 text-center text-[11px] text-gray-400">
                {previewBusy ? 'Оновлюю точний кадр…' : draft.publish_type === 'reel' ? `Reel зі слайдів · ${draft.image_idx.length} фото` : draft.publish_type === 'story' ? 'Story 9:16' : draft.image_idx.length > 1 ? `Альбом · ${draft.image_idx.length} фото · перше буде обкладинкою` : 'Одинарний пост'}
              </div>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                {Object.entries(data.feed_presets).map(([key, value]) => (
                  <button key={key} type="button" onClick={() => setFeedPreset(key)}
                    className={`rounded-lg border px-2 py-2 text-xs transition ${draft.feed_preset === key ? 'border-[#1877F2] bg-blue-50 font-semibold text-[#1877F2] dark:bg-blue-900/20 dark:text-blue-300' : 'border-gray-200 text-gray-600 hover:border-blue-300 dark:border-gray-700 dark:text-gray-300'}`}>
                    {value.label}
                  </button>
                ))}
              </div></>}
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                {([['white', 'Біле'], ['soft', 'Світле'], ['dark', 'Темне']] as const).map(([key, label]) => (
                  <button key={key} type="button" onClick={() => setDraft(current => ({ ...current, background: key }))}
                    className={`rounded-lg border px-2 py-2 text-xs ${draft.background === key ? 'border-[#1877F2] font-semibold text-[#1877F2] dark:text-blue-300' : 'border-gray-200 text-gray-500 dark:border-gray-700'}`}>{label}</button>
                ))}
              </div>
              ...
              {focusedFrame && <div className="mt-4 space-y-2 rounded-xl border border-gray-200 p-3 dark:border-gray-700">
                ...
                {draft.publish_type === 'feed' && data.feed_edge_adjusted?.[draft.feed_preset]?.[focusedFrame.image_idx] && (
                  <p className="rounded-lg bg-blue-50 px-2.5 py-2 text-[11px] leading-relaxed text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
                    Фото вже доходить до краю. Автоматичне зменшення вимкнено — масштаб {facebookDefaultZoom(data, 'feed', focusedFrame.image_idx, draft.feed_preset).toFixed(2)}. Його можна змінити вручну.
                  </p>
                )}
              </div>}
            ...
            <section className="space-y-4">
                ...
                {data.image_urls.length ? (
                  <div className="grid grid-cols-4 gap-2 sm:grid-cols-5">
                    {data.image_urls.map((url, index) => {
                      ...
                      return (
                        <div key={`${url}-${index}`} className={`relative overflow-hidden rounded-lg border-2 ${order ? 'border-[#1877F2]' : 'border-transparent'}`}>
                          ...
                          {order && <span className="absolute left-1 top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#1877F2] px-1 text-[10px] font-bold text-white">{order}</span>}
                          {order && <button type="button" onClick={event => { event.stopPropagation(); setFocusedImage(index); }} title="Налаштувати кадр" className={`absolute right-1 top-1 rounded px-1 text-[10px] text-white ${focusedImage === index || (focusedImage == null && order === 1) ? 'bg-[#1877F2]' : 'bg-black/60'}`}>⚙</button>}
                          {order && (
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label htmlFor="facebook-caption" className="text-xs font-semibold text-gray-700 dark:text-gray-200">{draft.publish_type === 'story' ? 'Текст на зображенні Story' : draft.publish_type === 'reel' ? 'Опис Reel' : 'Текст допису'}</label>
                  <span className={`text-xs ${(draft.publish_type === 'story' ? storyTextTooLong : captionTooLong) ? 'font-semibold text-red-500' : 'text-gray-400'}`}>
                    {draft.publish_type === 'story' ? `${draft.story_text.length}/${storyTextLimit}` : `${draft.caption.length}/${captionLimit}`}
                  </span>
                </div>
                <textarea id="facebook-caption" value={draft.publish_type === 'story' ? draft.story_text : draft.caption}
                  ...
                  className={`w-full resize-y rounded-xl border bg-white px-3 py-2.5 text-sm leading-relaxed text-gray-800 outline-none focus:ring-2 dark:bg-gray-800 dark:text-gray-100 ${(draft.publish_type === 'story' ? storyTextTooLong : captionTooLong) ? 'border-red-400 focus:ring-red-500/20' : 'border-gray-200 focus:border-[#1877F2] focus:ring-blue-500/20 dark:border-gray-700'}`} />
                <p className="mt-1 text-[11px] text-gray-400">{draft.publish_type === 'story'
                  ? 'BMS нанесе цей текст прямо на кадр; точний вигляд одразу видно в прев’ю.'
                  : 'Той самий товарний текст, що й в Instagram — одна крамниця, один голос.'}</p>
              </div>
              ...
              {draft.publish_type === 'reel' && <div className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2.5 text-xs leading-relaxed text-sky-800 dark:border-sky-800 dark:bg-sky-900/20 dark:text-sky-200">
                BMS автоматично створить відео 9:16 і показуватиме кожне вибране фото приблизно 2,5 секунди. Reel публікується без звуку: Pages API не вміє вибирати музику з бібліотеки Meta.
              </div>}

              {draft.publish_type === 'story' && <div className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2.5 text-xs leading-relaxed text-sky-800 dark:border-sky-800 dark:bg-sky-900/20 dark:text-sky-200">
              ...
              {data.condition_confirmation_required && <label className="flex gap-2 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-xs text-amber-900 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-200">
                ...
                <span>Підтверджую стан «{data.condition_name}» перед живою Facebook-публікацією.</span>
              </label>}
              {validation && (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 bg-gray-50/70 px-5 py-3.5 dark:border-gray-800 dark:bg-gray-950/30">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={validate} disabled={validating || busy || !draft.image_idx.length || captionTooLong || storyTextTooLong || (captionRequired && !draft.caption.trim())}
              className="inline-flex items-center gap-2 rounded-lg bg-[#0B5FCC] px-4 py-2 text-sm font-semibold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40">
...
export default FacebookPublishDialog;
