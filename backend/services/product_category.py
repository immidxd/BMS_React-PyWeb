...
_DRESS = re.compile(r"плат|сукн|комбінезон|сарафан|костюм")
...
def clothing_subcat(type_name: Optional[str]) -> str:
    """'bottom' | 'dress' | 'top' — визначає, які заміри мають сенс."""
    s = _key(type_name)
    if _BOTTOM.search(s):
        return "bottom"
    if _DRESS.search(s):
...
MEASUREMENT_STICKER_HINTS: Dict[str, str] = {
    ...
    "length": "довжина виробу; на стікері «д», «дл», «довж», «довжина»",
    "sleeve": "рукав (довжина); на стікері «р», «рук», «рукав»",
}
