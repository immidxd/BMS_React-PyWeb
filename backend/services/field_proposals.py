...
CONFIDENCE_THRESHOLD: Dict[str, float] = {
    ...
    "manufacturer_country_name": 0.85,  # читається з бирки; головний ризик — країна бренда замість «Made in»
    "style_name":          0.75,
    "season":              0.75,   # доповнює наявне, нічого не прибирає — ціна помилки низька
    "subtype_name":        0.80,
    # Зі стікера від руки — per-item, помилка найдорожча (ціна їде в Журнал і
    # на маркетплейси). Плюс окремий запобіжник: номер на стікері = номер картки.
    # Стікер рукописний і часто повернутий: на читабельному «2500» модель
    # дає 0.8. Людина підтверджує чіп, дивлячись на той самий стікер, а межі
    # значень (STICKER_FIELDS) відсікають хибні читання.
    "price":               0.75,
    "sizeeu":              0.75,
    "measurementscm":      0.70,
    ...
    "size_letter":         0.75,
    # Примітка з розкладом довжини костюма (кофта/штани) — доповнює наявну,
    # той самий поріг, що й самі заміри зі стікера.
    "extranote":           0.70,
