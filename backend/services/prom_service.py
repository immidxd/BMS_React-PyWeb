...
# Гендерозалежна назва атрибута розміру (унісекс → чоловіча, як категоризує Prom).
_GENDER_SIZE_ATTR = {"жіноча": "Размер женской обуви", "чоловіча": "Размер мужской обуви",
                     "унісекс": "Размер мужской обуви", "дитяча": "Размер детской обуви"}
...
def _build_params(bms: dict) -> List[tuple]:
    ...
    size_attr = _GENDER_SIZE_ATTR.get(gl, "Размер мужской обуви")  # Розмір (усі доступні)
    for sz in (bms.get("sizes") or []):
        out.append((size_attr, str(sz)))
    if color:                                                   # Цвет
