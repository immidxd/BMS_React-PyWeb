...
Ключ зіставлення
────────────────
Meta показує в історії платежів контрольний номер (напр. `VP93D4ECP4`), і рівно
він же стоїть в описі банківської операції: `FACEBK *VP93D4ECP4`. Це наскрізний
ключ, тож зіставлення точне, а не «та сама дата й схожа сума». Якщо коду немає —
відкат на (дата + сума в USD), і такий збіг позначається окремо як слабший.
...
_META_DESCRIPTION = re.compile(r"\bFACEB(?:K|OOK)\b", re.IGNORECASE)
