...
async def story_automation_approve_draft(
    ...
    """Відправити Story тим самим шляхом, що й автоматика без перевірки."""
    try:
        from services.auto_publish import publish_story_draft, PublishRefused
    except ImportError:
        from backend.services.auto_publish import publish_story_draft, PublishRefused
    try:
        return await publish_story_draft(db, draft_id, body=payload or {})
...
async def collection_automation_approve_draft(
    ...
    """Відправити підбірку тим самим шляхом, що й автоматика без перевірки."""
    try:
        from services.auto_publish import publish_collection_draft, PublishRefused
    except ImportError:
        from backend.services.auto_publish import publish_collection_draft, PublishRefused
    try:
        result = await publish_collection_draft(db, draft_id, body=payload or {})
    except PublishRefused as exc:
        db.rollback()
        raise HTTPException(status_code=exc.status, detail=str(exc))
