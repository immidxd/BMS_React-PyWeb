...
def test_fine_texture_is_smooth_not_ribbed():
    ...
    assert "всій площі" in h["рифлена"]
    assert "дрібн" not in h["рифлена"]                             # «дрібні» більше не означення рифленої
    assert "НЕГЛИБОКИЙ" in h["__field__"] and "«гладка»" in h["__field__"]
    assert "ботильйони" in h["__field__"]


def test_relief_is_the_default_for_any_real_tread():
    """Третє уточнення власника (EA7 — еталон «рифленої»): рифлена = густі
    паралельні ребра/зубці; усе інше з виразним протектором, що не лінії й не
    глибокі шашки, — «рельєфна» за замовчуванням; неглибокі малюнки — гладка."""
    h = pa.VALUE_HINTS["tread_type"]
    assert "УСЕ ІНШЕ" in h["__field__"] and "«рельєфна»" in h["__field__"]
    assert "НЕГЛИБОКИЙ" in h["__field__"] and "«гладка»" in h["__field__"]
    assert "паралельні ребра" in h["рифлена"] and "EA7" in h["рифлена"]
    assert "будь-який ВИРАЗНИЙ протектор" in h["рельєфна"] and "за сумніву" in h["рельєфна"]
    assert "неглибокий малюнок протектором не вважається" in h["гладка"]
...
def test_blocks_and_chevrons_are_relief_not_ribbed():
    ...
    h = pa.VALUE_HINTS["tread_type"]
    assert "паралельні ребра" in h["рифлена"] and "жодних фігур" in h["рифлена"]
    assert "блоки" in h["рифлена"] and "«рельєфна»" in h["рифлена"]     # межа названа прямо
    assert "блоки" in h["рельєфна"] and "шеврони" in h["рельєфна"]
    assert "ШИРОКИМИ проміжками" in h["тракторна"]
    assert "паралельні ребра" in h["__field__"] and "шашки" in h["__field__"]
